...
########################################
    ...
    eval { $autobanIndexStatus = $autoban::esMgmt->indices->exists( index => $autoban::autobanConfig->param('autoban.esAutobanIndex') ); };
    ...
    unless ($autobanIndexStatus) {
        ...
        eval { $autoban::esMgmt->indices->create( index => $autoban::autobanConfig->param('autoban.esAutobanIndex') ); };
        ...
        eval { $autobanIndexState = $autoban::esMgmt->cluster->state( index => $autoban::autobanConfig->param('autoban.esAutobanIndex'), metric => 'metadata' ) };
        ...
        if ( $autobanIndexState->{'metadata'}->{'indices'}->{ $autoban::autobanConfig->param('autoban.esAutobanIndex') }->{'state'} ne 'open' ) {
            ...
            eval { $autoban::esMgmt->indices->open( index => $autoban::autobanConfig->param('autoban.esAutobanIndex') ); };
...
########################################
    ...
    unless ( $autoban::autobanConfig->param('autoban.minEsClusterHealth') eq 'off' ) {
        ...
        eval { $clusterHealth = $autoban::esLogging->cluster->health(); };
        autoban::Logging::OutputHandler( 'ERROR', 'autoban', "Problem connecting to elasticsearch: $@" ) if $@;
